...
This definition of centralization focuses largely upon the people who have influence upon or access to a function. While centralization is also a concern from a purely technical standpoint (for example, designing a resilient cloud service), this more political focus deserves separate treatment, because anticipating the failure modes involving rational actors requires strategic thinking.

As a result, the concept of availability is distinct from centralization, and any relationship between them cannot be assumed without careful analysis of where centralization occurs. Centralized systems might be more available due to factors like the resources available to them, but also have greater impact when they have a fault; decentralized systems might be more resilient in the face of local failures, but less able to react to systemic issues.
