...
For example, consider the impact of centralization upon availability. A centrally operated system might be more available because of factors like the resources available to a larger operator, but also have greater impact when a fault is encountered. On the other hand, decentralized systems might be more resilient in the face of some forms of failure, but more prone to disruption due to others; for example, they may be less able to react to systemic issues, and might be exposed to a larger collection of security vulnerabilities in total.
